...
class TraceWindow(BaseWindow):
    ...
    def setClassifications(trace, yi_pred):
        ...
        (
            ...
            trace.confidence,
            trace.first_bleach,
        ) = deepfret_nm.main.python.lib.math.seq_probabilities(trace.y_pred)
    ...
    def classifyTraces(self, single=False, checked_only=False):
        ...
        if len(traces) > 0:
            ...
            if not single:
                all_lengths_eq = deepfret_nm.main.python.lib.math.all_equal(
                    [trace.frames_max for trace in traces]
                )
                all_features_eq = deepfret_nm.main.python.lib.math.all_equal(
            ...
            if all((all_lengths_eq, all_features_eq)):
                ...
                X = np.array(
                    [
                        deepfret_nm.main.python.lib.math.correct_DA(
                ...
                X = (
                    X[..., [1, 2]]
                    if deepfret_nm.main.python.lib.math.contains_nan(X[..., -1])
                    else X[..., [1, 2, 3]]
                )
                # Normalize tensor
                X = deepfret_nm.main.python.lib.math.sample_max_normalize_3d(X)
                ...
                Y = deepfret_nm.main.python.lib.math.predict_batch(
                    X=X,
                    model=model,
                    progressbar=progressbar,
                ...
                for n, trace in enumerate(traces):
                    ...
                    if deepfret_nm.main.python.lib.math.contains_nan(xi[..., -1]):
                    ...
                    xi = deepfret_nm.main.python.lib.math.sample_max_normalize_3d(X=xi)

                    yi = deepfret_nm.main.python.lib.math.predict_single(xi=xi, model=model)
                    Y.append(yi)
                    if n % batch_size == 0:
                        progressbar.increment()
        ...
        if self.currName is not None:
            for n, ax in enumerate(self.canvas.axes):
                if ax == event.inaxes and n in [1, 2]:
                    ...
                    self.currentTrace().first_bleach = deepfret_nm.main.python.lib.math.min_real(
    ...
    def sortListByCondition(self, setup):
        ...
        for index in range(self.listModel.rowCount()):
            ...
            if setup in ("red", "green"):
                ...
                if deepfret_nm.main.python.lib.math.contains_nan(trace.red.int):
                    cond1 = True
                else:
    ...
    def getCorrectionFactors(self, factor):
        ...
        if self.currName is not None and len(trace.xdata) == 2:
            ...
            if factor == "alpha":
                trace.a_factor = deepfret_nm.main.python.lib.math.alpha_factor(I_DD, I_DA)
            elif factor == "delta":
                trace.d_factor = deepfret_nm.main.python.lib.math.delta_factor(I_DD, I_DA, I_AA)
            self.refreshPlot()
    ...
    def refreshPlot(self):
        ...
        if trace is not None and len(self.data.traces) > 0:
            ...
            delta = self.getConfig(gvars.key_deltaFactor)
            factors = alpha, delta
            F_DA, I_DD, I_DA, I_AA = deepfret_nm.main.python.lib.math.correct_DA(
            ...
            for (ax, label), channel, color in zip(
                ...
                if not deepfret_nm.main.python.lib.math.contains_nan(signal):
                    deepfret_nm.main.python.lib.plotting.set_axis_exp_ylabel(
            ...
            fret = deepfret_nm.main.python.lib.math.calc_E(trace.get_intensities(), *factors)
            stoi = deepfret_nm.main.python.lib.math.calc_S(trace.get_intensities(), *factors)
            ...
            if hasattr(self.canvas, "ax_ml") and trace.y_pred is not None:
                deepfret_nm.main.python.lib.plotting.plot_predictions(
