...
class HistogramWindow(BaseWindow):
    ...
    def getHistogramData(self, n_first_frames="spinbox"):
        ...
        for trace in checkedTraces:
            E, S = deepfret_nm.main.python.lib.math.drop_bleached_frames(
        ...
        self.E_un, self.S_un = deepfret_nm.main.python.lib.math.trim_ES(E_app, S_app)

        # Skip ensemble correction if stoichiometry is missing
        if not deepfret_nm.main.python.lib.math.contains_nan(self.S_un):
            if len(self.E_un) > 0:
                beta, gamma = deepfret_nm.main.python.lib.math.beta_gamma_factor(
                ...
                for trace in checkedTraces:
                    ...
                    E_real.extend(E)
                    S_real.extend(S)
                self.E, self.S = deepfret_nm.main.python.lib.math.trim_ES(E_real, S_real)
                self.beta = beta
                self.gamma = gamma
        else:
            self.E = self.E_un
    ...
    def fitGaussians(self, states):
        ...
        if E is not None:
            ...
            best_model, params = deepfret_nm.main.python.lib.math.fit_gaussian_mixture(
                X=E,
    ...
    def plotTop(self, corrected):
        ...
        if self.gauss_params is not None:
            ...
            for (m, s, w) in self.gauss_params:
                _, y = deepfret_nm.main.python.lib.plotting.plot_gaussian(
    ...
    def plotCenter(self, corrected):
        ...
        if S is not None:
            c = deepfret_nm.main.python.lib.math.contour_2d(
                xdata=E,
                ydata=S,
