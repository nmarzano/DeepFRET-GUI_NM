...
class TransitionDensityWindow(BaseWindow):
    ...
    def plotTransitionDensity(self, params):
        ...
        if (
            ...
            cont = deepfret_nm.main.python.lib.math.contour_2d(
            ...
            self.colors = deepfret_nm.main.python.lib.plotting.get_colors(
    ...
    def plotHistograms(self):
        ...
        if self.data.tdpData.df is not None:
            ...
            bw = deepfret_nm.main.python.lib.math.estimate_binwidth(fret_lifetimes)
            bins = np.arange(0, max_lifetime, bw)

            for k, cluster in self.data.tdpData.df.groupby("label"):
                try:
                    hx, hy, *_ = deepfret_nm.main.python.lib.math.histpoints_w_err(
                    ...
                    popt, pcov = scipy.optimize.curve_fit(
                        deepfret_nm.main.python.lib.math.exp_function, xdata=hx, ydata=hy
                    )
                    ...
                    self.hist_axes[k].plot(
                        bins,
                        deepfret_nm.main.python.lib.math.exp_function(bins, *popt),
                        "--",
                        color="black",
                        label="label: {}\n"
                        "$\lambda$:  ${:.2f} \pm {:.2f}$\n"
