...
class TraceContainer:
    ...
    def load_from_txt(self):
        ...
        # Else DeepFRET trace compatibility
            ...
            df = deepfret_nm.main.python.lib.utils.csv_skip_to(
                path=self.filename, line="D-Dexc", sep="\s+"
            )
        try:
            pair_n = deepfret_nm.main.python.lib.utils.seek_line(
        ...
        try:
            video = deepfret_nm.main.python.lib.utils.seek_line(
            ...
            pass
        try:
            bleach = deepfret_nm.main.python.lib.utils.seek_line(
            ...
            if "p_bleached" in df.columns:
                ...
                try:
                    ...
                    (
                        ...
                        self.confidence,
                        _,
                    ) = deepfret_nm.main.python.lib.math.seq_probabilities(self.y_pred)
                except KeyError:
                    pass
    ...
    def get_tracename(self) -> str:
        ...
        if self.tracename is None:  # define the tracename if it doesn't exist
            self.tracename = os.path.basename(
                deepfret_nm.main.python.lib.utils.remove_newlines(self.filename)
            )
    ...
    def calculate_fret(self):
        ...
        """
        self.fret = deepfret_nm.main.python.lib.math.calc_E(self.get_intensities())
    ...
    def calculate_stoi(self):
        ...
        :return:
        """
        self.stoi = deepfret_nm.main.python.lib.math.calc_S(self.get_intensities())
    ...
    def load_video_data(
        ...
        if view_setup == gvars.key_viewsetupInterleaved:
            ...
            if view_setup == gvars.key_viewSetupQuad:
                top, btm, lft, rgt = deepfret_nm.main.python.lib.imgdata.quadrant_indices(
            ...
            lft, rgt = deepfret_nm.main.python.lib.imgdata.left_right_indices(width=video.width)
        ...
        for c in video.grn, video.red, video.acc:
            if c.raw is not None:
                ...
                c.mean = deepfret_nm.main.python.lib.imgdata.zero_one_scale(c.mean)
                if bg_correction:
                    c.mean_nobg = deepfret_nm.main.python.lib.imgdata.subtract_background(
